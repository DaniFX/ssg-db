...
func (r *ServiceEndpointRepository) GetActiveByServiceID(ctx context.Context, serviceID string) ([]models.ServiceEndpoint, error) {
	iter := r.client.Collection(r.collection).Where("serviceId", "==", serviceID).Where("isActive", "==", true).Documents(ctx)
	defer iter.Stop()
...
func (r *ServiceEndpointRepository) Update(ctx context.Context, endpoint *models.ServiceEndpoint) error {
	endpoint.UpdatedAt = time.Now()
	_, err := r.client.Collection(r.collection).Doc(endpoint.ID).Set(ctx, endpoint)
	return err
}
